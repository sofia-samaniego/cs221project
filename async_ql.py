...
# Train agent
################################################################################
def train_thread(sess, thread_id, env):
    """Trains the RL Agent"""
...
def test(thread_id, env):
    time.sleep(3*thread_id)
    print 'hi from threadID {}'.format(thread_id)
    tempList = []
    tempList.append(thread_id)

    if tempList:
        print tempList
    print 'not tempList: {}'.format(tempList)

def train(sess):

    env = [EnvWrapper(gym.make(GAME), BUFFER_SIZE) for i in range(NUM_THREADS)]

    #threads = [threading.Thread(target=train_thread, args(sess, thread_id, env[thread_id]) \
            #for thread_id  in range(NUM_THREADS)]

    threads = [threading.Thread(target=test, args = (thread_id, env[thread_id])) \
            for thread_id  in range(NUM_THREADS)]
